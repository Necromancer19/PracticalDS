...
for l in range(2,8):
    for w in range(2,8):
        for h in range(4):
            ...
            if t==1:
                PalletFrame = pd.DataFrame(columns=['IDNumber', 'ShipType', 'UnitNumber', 'Box_per_Length', 'Box_per_Width', 'Box_per_Height'])
            PalletRow = pd.DataFrame.from_dict([PalletLine])
            PalletFrame = pd.concat([PalletFrame, PalletRow], ignore_index=True)
...
sSQL=sSQL+ " P.PalletNumber AS PalletNumber,"
sSQL=sSQL+ " B.UnitNumber AS BoxNumber,"
